...
public class PeopleWidget extends AppWidgetProvider
{
	...
	private void updateBoard(AppWidgetManager appWidgetManager, int appWidgetId)
	{
		...
		mWidget.removeAllViews(R.id.recent_contacts_row_1);
		mWidget.removeAllViews(R.id.recent_contacts_row_2);
		mWidget.removeAllViews(R.id.recent_contacts_row_3);
		...
		updateRecentContactsList(mContext, mWidget, lastContacted);
	...
	private void updateRecentContactsList(Context context, RemoteViews widget, List<ContactInfo> contactInfoList)
	{
		int viewCounter = 0;
		int limit = PeopleManager.getInstance().getLastContactedLimit();

		// Add one to the limit to make room for the all contacts button
		limit = (limit % 3 == 0) ? limit : (limit + 1);
		...
		for (ContactInfo contactInfo : contactInfoList)
		{
			...
			if (view != null)
			{
				if (viewCounter <= 1)
				{
					widget.addView(R.id.recent_contacts_row_1, view);
				}
				else if (viewCounter <= 3)
				{
					widget.addView(R.id.recent_contacts_row_2, view);
				}
				else
				{
					widget.addView(R.id.recent_contacts_row_3, view);
				}
				viewCounter++;
			}
		}
		...
		{
			widget.addView(R.id.recent_contacts_row_1, allContactsView);
		}
		else if (contactInfoList.size() <= 3)
		{
			widget.addView(R.id.recent_contacts_row_2, allContactsView);
		}
		else
		{
			widget.addView(R.id.recent_contacts_row_3, allContactsView);
		}
	}
